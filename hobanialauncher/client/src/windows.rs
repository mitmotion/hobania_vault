    ...
    if let Some(latest_release) = releases.first() {
        ...
        if Version::parse(&latest_release.version)?
            ...
            && latest_release
                ...
                .is_some()
        {
            tracing::debug!("Found new Hobanialauncher release: {}", &latest_release.version);
            return Ok(Some(latest_release.clone()));
        } else {
            tracing::debug!("Hobanialauncher is up-to-date.");
        }
    }
    Ok(None)
}
    ...
    if let Some(asset) = asset {
        ...
        let result = windows::execute_as_admin(
            ...
            &format!(
                ...
                update_cache_path.join("hobanialauncher-install.log").display()
            ),
        );

        if result <= 32 {
            tracing::error!(
                "Failed to update hobanialauncher! {}",
