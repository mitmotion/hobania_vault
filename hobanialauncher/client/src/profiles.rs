...
impl Server {
    pub fn url(&self) -> &str {
        match self {
            Server::Production => "https://download.hobania.mitmotion.co.za",
            Server::Staging => "https://download.staging.hobania.mitmotion.co.za",
            Server::Test => "https://download.test.hobania.mitmotion.co.za",
        }
    }
}
...
impl Profile {
    ...
    /// e.g. <base>/profiles/default/hobania-voxygen.exe
    ...
    pub fn start(profile: &Profile, game_server_address: Option<&str>) -> Command {
        ...
        envs.insert("HOBANIA_USERDATA", userdata_dir);
        envs.insert("HOBANIA_ASSETS", assets_dir);
