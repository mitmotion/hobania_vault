...
pub fn run(cmd: CmdLine) -> Result<()> {
    Ok(Hobanialauncher::run(settings(cmd))?)
}

#[derive(Default, Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct Hobanialauncher {
    #[serde(skip)]
    view: View,
...
impl Hobanialauncher {
                ...
                Ok(file) => {
                    match ron::de::from_reader(file) {
                        Ok(state) => {
                            ...
                            let mut state: Hobanialauncher = state;
                            state.cmd = flags;
                            state
                        },
                        Err(e) => {
    ...
    pub async fn save(hobanialauncher: Self) -> Result<()> {
        let data = tokio::task::block_in_place(|| {
            ron::ser::to_string_pretty(&hobanialauncher, PrettyConfig::default())
        })?;
...
#[derive(Clone, Debug)]
pub enum Message {
    Loaded(Hobanialauncher), // Todo: put in Box<>
    Saved(Result<()>),
...
impl Application for Hobanialauncher {
    ...
    fn new(flags: CmdLine) -> (Self, Command<Message>) {
        ...
        (
            Hobanialauncher::new(&flags),
            Command::perform(Self::load(flags.clone()), Message::Loaded),
        )
    }

    fn title(&self) -> String {
        format!("Hobanialauncher v{}", env!("CARGO_PKG_VERSION"))
    }
...
fn settings(cmd: CmdLine) -> Settings<CmdLine> {
    ...
    Settings {
        ...
        id: Some("hobanialauncher".to_string()),
