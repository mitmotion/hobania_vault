//! Display an update dialog (windows only) to ask whether to update hobanialauncher
...
impl Default for UpdateView {
    fn default() -> Self {
        Self {
            message: "Update for Hobanialauncher available. Do you want to update?"
                .to_string(),
        }
    }
}
    ...
    pub fn update(
        ...
        match msg {
            ...
            UpdateViewMessage::UpdatePressed => {
                tracing::info!("Updating Hobanialauncher...");
                self.message = "Updating Hobanialauncher...".to_string();
                let release = release.as_ref().unwrap().clone();
                return Command::perform(
                    async {
                        tokio::task::block_in_place(move || {
                            if let Err(e) = crate::windows::update(&release) {
                                tracing::error!("Failed to update Hobanialauncher: {}", e);
                                return e.to_string();
                            }
                            String::new()
                        })
