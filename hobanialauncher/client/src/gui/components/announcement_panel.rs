use crate::{
    ...
    gui::{
        style::{HobanialauncherDownloadButtonStyle, AnnouncementStyle, DARK_WHITE},
        views::{
...
impl AnnouncementPanelComponent {
    ...
    pub fn view(&self) -> Element<DefaultViewMessage> {
        ...
        let rowtext = match (update, &self.announcement_message) {
            ...
            (true, None) => {
                "Hobanialauncher is outdated, please update to the latest release!".to_string()
            },
            (false, Some(msg)) => {
            ...
            (true, Some(msg)) => {
                format!("Hobanialauncher is outdated! News: {}", msg)
            },
        };
            ...
            content_row = content_row.push(
                container(
                    button(
                        row()
                            .push(
                                text("Download Hobanialauncher").color(Color::WHITE).size(14),
                            )
                    ...
                    .style(HobanialauncherDownloadButtonStyle),
                )
