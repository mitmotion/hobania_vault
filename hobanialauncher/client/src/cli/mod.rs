...
pub fn process() -> Result<()> {
    ...
    tracing::info!("Visit https://book.hobania.mitmotion.co.za/ for an FAQ and Troubleshooting");
    ...
    // let the user know incase hobanialauncher can be updated.
    #[cfg(windows)]
    if let Ok(Some(release)) = crate::windows::query() {
        tracing::info!(
            "New Hobanialauncher release found: {}. Run `hobanialauncher upgrade` to update.",
            release.version
        );
    }

    rt.block_on(async {
        let mut state = Hobanialauncher::load(cmd.clone()).await;
...
async fn start(profile: &mut Profile, game_server_address: Option<String>) -> Result<()> {
    if !profile.installed() {
        tracing::info!("Profile is not installed. Install it via `hobanialauncher update`");
        return Ok(());
    }
    ...
    while let Some(progress) = stream.next().await {
        match progress {
            io::ProcessUpdate::Line(line) => tracing::info!("[Hobania] {}", line),
            io::ProcessUpdate::Exit(exit) => {
                tracing::info!("Hobania exited with {}", exit)
            },
...
fn upgrade() -> Result<()> {
    match crate::windows::query()? {
        Some(release) => {
            tracing::info!("Found new Hobanialauncher release: {}", release.version);
            crate::windows::update(&release)?;
        },
        None => tracing::info!("Hobanialauncher is up-to-date."),
    }
    Ok(())
}
